...
namespace widemovement001 {
    /// <summary>
    /// 移動が可能かの判定・幅のある移動
    /// </summary>
    /// <remarks>https://paiza.jp/works/mondai/a_rank_level_up_problems/a_rank_snake_mapmove_step4/edit?language_uid=c-sharp</remarks>
    class Program {
        static void Main() {
            // 条件入力
            ...
            foreach (var m in moves) {
                var addP = GetAddPoint(ref direction, m.Key);
                var isStopped = false;
                for (var i = 0; i < m.Value; i++) {
                    ...
                    if ((0 <= current_x && current_x <= width - 1) &&
                        ...
                        // 1つ前に戻してから表示
                        Console.WriteLine($"{current_y - addP.Y} {current_x - addP.X}");
